...
**Статус внешней проверки:** ожидается настройка Docker Hub и секретов. Успешная публикация и этап Deploy пока не подтверждены. Ссылку на успешный запуск и digest следует добавить после фактического выполнения; вымышленные результаты не приводятся.
